...
/*		for(i=lside;i<rside;i++)
	...
	if (death == 4095)
	{
	    ...
	    if ((newkeystatus[newkeydefs[9]]|newkeystatus[newkeydefs[10]]|newkeystatus[newkeydefs[11]]|newkeystatus[newkeydefs[20]]) > 0)
	    {
		j = bulchoose;
        ...
        if (newkeystatus[newkeydefs[20]] > 0 && ((totalclock^ototclock) > 32)) {
            	i = 2;
            	do {
	...
	if (death == 4095)
	{
		...
		if (newkeystatus[newkeydefs[18]] > 0)
		{
		    svel += (clockspd<<2);
		    if (svel > maxvel)
			svel = maxvel;
